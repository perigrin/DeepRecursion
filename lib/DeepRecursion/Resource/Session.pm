...
sub GET {
    ...
    if ( $self->request->path_info =~ qr|/session| ) {
        ...
        my $session_id = ( split '/', $self->request->path_info )[-1];
        if ( $session->id eq $session_id ) {
            $self->response->redirect('/');
            return DONE;
        }
        else {
            $self->set_error( {status_code => 404, reason => "Couldn't find session $session_id"});
            return OK;
        }
    }
    return OK;
...
sub POST {
    ...
    unless ($user) {
        $self->set_error( {status_code => 404, reason => "Couldn't find user $username."});
        return OK;
    }
